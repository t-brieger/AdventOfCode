...
public static class GeneralRunner
{
	...
	private static async Task RunSolution(Solution s, byte d, ushort y, bool test, int progressWidth)
	{
		...
		if (test)
		{
			...
			{
				string testInput = parts[i];
				s!.rawInput = testInput;
				input = testInput.Trim();

				string part1Expected = parts[i + 1].Trim();
				string part2Expected = parts[i + 2].Trim();

				string part1Ans = s.Part1(input).Trim();
				string part2Ans = s.Part2(input).Trim();

				if (part1Ans == part1Expected && part2Ans == part2Expected)
				{
					Console.ForegroundColor = ConsoleColor.Green;
					Console.WriteLine($"\tTest {i / 3 + 1} passed.");
					Console.ResetColor();
				}
				else if (part1Ans == part1Expected && part2Ans != part2Expected)
				{
					Console.ForegroundColor = ConsoleColor.Red;
					Console.Write($"\tTest {i / 3 + 1}: ");
					Console.ForegroundColor = ConsoleColor.Green;
					Console.Write($"Correct Answer for Part 1, ");
					Console.ForegroundColor = ConsoleColor.Red;
					Console.WriteLine($"Incorrect for Part 2 (expected: {part2Expected}, got: {part2Ans}).");
					Console.ResetColor();
				}
				else if (part1Ans != part1Expected && part2Ans == part2Expected)
				{
					Console.ForegroundColor = ConsoleColor.Red;
					Console.Write($"\tTest {i / 3 + 1}: ");
					Console.Write($"Incorrect Answer for Part 1 (expected: {part1Expected}, got: {part1Ans}),  ");
					Console.ForegroundColor = ConsoleColor.Green;
					Console.WriteLine("Correct for Part 2.");
					Console.ResetColor();
				}
				else
				{
					Console.ForegroundColor = ConsoleColor.Red;
					Console.WriteLine($"\tTest {i / 3 + 1}: Both parts wrong (expected: {part1Expected} / {part2Expected}, got: {part1Ans} / {part2Ans}).");
					Console.ResetColor();
				}
			}
		...
		try
		{
			(Action p1Inc, Action<int> p1Res, Action<int> p1Set, Func<int> p1Get) =
				MakeProgressBar(progressWidth, $"{y} / {d,2} Part 1");
			s.IncreaseBar = p1Inc;
			s.RescaleBar = p1Res;
			s.SetBar = p1Set;
			s.GetBarScale = p1Get;
		...
		try
		{
			(Action p2Inc, Action<int> p2Res, Action<int> p2Set, Func<int> p2Get) =
				MakeProgressBar(progressWidth, $"{y} / {d,2} Part 2");
			s.IncreaseBar = p2Inc;
			s.RescaleBar = p2Res;
			s.SetBar = p2Set;
			s.GetBarScale = p2Get;
	...
	private static (Action inc, Action<int> rescale, Action<int> set, Func<int> get) MakeProgressBar(int terminalWidth,
		string label)
	{
		int done = -1;
		int max = 1;
		...
		Action inc = () =>
		{
			...
			Console.Write(
				$"\r{label}: [{new string('#', filledCount)}{new string(' ', terminalWidth - filledCount)}] ({percentage,3}%)");
		};
