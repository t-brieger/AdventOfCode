...
namespace AdventOfCode.Solutions
{
    public class Year2018Day11 : Solution
    {
        private static int BuildSaTable(int[,] a, int x, int y)
        {
            ...
            return x switch
            {
                >= 1 when y == 0 => a[x, y] + a[x - 1, y],
                0 when y >= 1 => a[x, y] + a[x, y - 1],
                0 => a[x, y],
                _ => a[x, y] + a[x, y - 1] + a[x - 1, y] - a[x - 1, y - 1]
            };
        }
        ...
        public override string Part2(string input)
        {
            ...
            {
                int a = powerLevel[i + size, j + size] - powerLevel[i + size, j] - powerLevel[i, j + size] +
                        powerLevel[i, j];
