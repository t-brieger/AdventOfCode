...
public class Year2018Day03 : Solution
{
    public override string Part1(string input)
    {
        ...
        List<short[]> tmpList = tmp2
            .Select(s => s.Split(new[] { '#', ' ', '@', ':', 'x', ',' }, StringSplitOptions.RemoveEmptyEntries))
            .Select(splitStrings => new[]
            {
                Int16.Parse(splitStrings[0]), Int16.Parse(splitStrings[1]), Int16.Parse(splitStrings[2]),
                Int16.Parse(splitStrings[3]), Int16.Parse(splitStrings[4])
            }).ToList();
    ...
    public override string Part2(string input)
    {
        ...
        for (short i = 0;
             i < inputs.Length / 5;
             i++) //inputs is a 2d array using the [*,*] syntax instead of [*][*], so it's a bit awkward to loop through
        {
        ...
        for (short j = 0; j < 1001; j++)
        {
            ...
            foreach (short claim in gridClaims[i, j]) canBeIntact[claim] = false;
        }
