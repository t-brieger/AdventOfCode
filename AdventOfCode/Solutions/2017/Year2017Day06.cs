...
namespace AdventOfCode.Solutions
{
    ...
    {
        //TODO: un-jankify
        private static veryLongTuple GetLongAssTupleFromArray(IReadOnlyList<int> state)
        {
            return new veryLongTuple(state[0], state[1], state[2], state[3], state[4], state[5], state[6],
                new Tuple<int, int, int, int, int, int, int, Tuple<int, int>>(state[7], state[8], state[9], state[10],
                    state[11], state[12], state[13], new Tuple<int, int>(state[14], state[15])));
        }

        public override string Part1(string input)
        {
            HashSet<veryLongTuple> seen = new();
            ...
            while (true)
            {
                ...
                currentState[max] = 0;
                for (int i = 1; i <= blocksToDistribute; i++) currentState[(max + i) % currentState.Length]++;
        ...
        {
            Dictionary<veryLongTuple, int> seen = new();
